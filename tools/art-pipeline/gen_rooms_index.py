...
def main():
    ...
    # bake locks onto exits + verify no room is orphaned by its locks
    locks = cfg.get('locks', [])
    for lk in locks:
        for e in out.get(lk['room'], {}).get('exits', []):
            if e['edge'] == lk['edge']:
                e['lock'] = {'flag': lk['flag'], 'msg': lk['msg']}
    # connectivity check treating locked edges as absent; grant rooms must
    # remain reachable so every flag is obtainable
    locked = {(lk['room'], lk['edge']) for lk in locks}
    adj = {}
    for name2, r2 in out.items():
        adj[name2] = [e2['to'] for e2 in r2['exits']
                      if e2['to'] in out and (name2, e2['edge']) not in locked]
    seen2, q2 = {'anchorroom'}, ['anchorroom']
    while q2:
        c2 = q2.pop()
        for t2 in adj.get(c2, []):
            if t2 not in seen2:
                seen2.add(t2)
                q2.append(t2)
    for lk in locks:
        assert lk['grant_room'] in seen2, \
            f"lock {lk['flag']}: grant room {lk['grant_room']} unreachable without the flag!"
    print(f'locks: {len(locks)} baked; all grant rooms reachable without their flags '
          f'({len(seen2)}/{len(out)} rooms open-world)')
    # world grid layout for stitched outdoor rendering
    layout = cfg.get('layout', {})
    interiors_set = set(cfg.get('interiors', {}).keys())
    layout_js = json.dumps(layout, indent=1)
    interior_list_js = json.dumps(sorted(interiors_set))
    js = ('// GENERATED by tools/art-pipeline/gen_rooms_index.py - do not hand-edit.\n'
          ...
          'export const WORLD_LAYOUT = ' + layout_js + ';\n'
          'export const INTERIOR_ROOMS = ' + interior_list_js + ';\n')
