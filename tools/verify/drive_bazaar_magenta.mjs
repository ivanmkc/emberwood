...
async function engineCoords(page) {
  return await page.evaluate(() => {
    ...
    for (let y = 0; y < H; y++)
      for (let x = 0; x < W; x++)
        blocked[y * W + x] = window.__ewWorld.blocked((window.__driveWoff ?? -640) + x, y) ? 1 : 0;
    const freeAt = (px, py) => {
    ...
    const spawn = [Math.round(window.__ew.player.x) - (window.__driveWoff ?? -640), Math.round(window.__ew.player.y)];
    const bandDefs = {
...
async function boot() {
  ...
  await page.evaluate(([r, w]) => { window.__driveRoom = r; window.__driveWoff = w; }, [ROOM, WOFF]);
...
const wanted = Object.keys(C.hasField).length;
console.log('fields:', routes.join(', '), `(${routes.length}/${wanted})`);
if (routes.length < wanted) { console.error('ABORT: distance field missing'); process.exit(1); }
for (const edge of routes) {
  ...
  const res = await page.evaluate((e) => {
    const W = 640, H = 448, WOFF = window.__driveWoff ?? -640;
      ...
      for (let py = 0; py < H - 15; py++)
        for (let px = 0; px < W - 12; px++) {
          ...
          for (const [hx, hy] of [[px+4,py+7],[px+11,py+7],[px+4,py+14],[px+11,py+14],[px+8,py+14]])
            if (window.__ewWorld.blocked(hx + WOFF, hy)) { ok = false; break; }
          hf[py * W + px] = ok ? 1 : 0;
        }
      window.__hitfree = hf;
    }
    ...
    const sx = Math.round(window.__ew.player.x) - WOFF;
    ...
    for (const cur of path) {
      window.__ew.player.x = (cur % W) + WOFF;
      window.__ew.player.y = (cur / W) | 0;
    }
    const endp = path[path.length - 1];
    return { ok: true, steps: path.length, px: (endp % W) + WOFF, py: (endp / W) | 0 };
  }, edge);
  ...
  await setPos(C.spawn[0] + WOFF, C.spawn[1]);
}
